...
exports.previewStaged = async (req, res) => {
  try {
    ...
    if (marketChange.change_type === 'update' && marketChange.originalValues) {
      // For updates, use the original values for the "before" scenario
      ...
      // For new products, "before" scenario doesn't include the new product
      beforeProducts = beforeProducts.filter(p => p.product_id !== marketChange.details.product_id);
    }
    ...
    if (prediction_method === 'fallback') {
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        try {
          ...
          // Store after decisions for commit
      ...
      // Calculate BEFORE scenario from existing dashboard data (agent memory + inventory)
      console.log('[INFO] Calculating BEFORE scenario from existing agent data');
      agents.forEach(agent => {
        // Process existing purchases from agent memory
        ...
        Object.entries(agent.current_inventory || {}).forEach(([pid, qty]) => {
          if (qty > 0) {
            ...
            if (product) {
              ...
              // Use average satisfaction for inventory items
              beforeSatisfaction.push(3.0);
            }
          }
        });
        
        // Calculate brand loyalty for before scenario
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        try {
          ...
          // AFTER scenario - call model with staged changes
          ...
          // Fallback to local decision maker for this batch
          const { makeImprovedFallbackDecisions } = require('../utils/improvedFallbackDecisionMaker');
      ...
      console.log(`[WARNING] Unknown prediction method: ${prediction_method}, using fallback`);
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        try {
          const beforeDecisions = await makeImprovedFallbackDecisions(agentProfiles, beforeRanked, JSON.stringify({ scenario: 'before_change' }));
          const afterDecisions = await makeImprovedFallbackDecisions(agentProfiles, afterRanked, JSON.stringify(marketChange.details));
          ...
          console.error(`[ERROR] Default fallback failed for batch ${i + 1}:`, error.message);
          failedBatches++;
        }
      }
    }
  ...
  try {
    ...
    if (marketChange.simulatedDecisions && marketChange.simulatedDecisions.length > 0) {
      for (const decision of marketChange.simulatedDecisions) {
        ...
        // Update inventory if purchase was made
